...
public class EventControllerIntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        adminLogin.setPassword("123123"); // Password from V3 migration script

        MvcResult loginResult = mockMvc.perform(post("/api/auth/signin")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(adminLogin)))
                // **IMPORTANT: Temporarily remove .andExpect(status().isOk())**
                // This allows the test to continue and print the failing response
        ...
        // Now, re-add the assertion, or assert on the 400 if you want to see the exact error.
        // For debugging, we just print first.
        // Once you've identified the root cause, you can put the .andExpect(status().isOk()) back.

        if (loginResult.getResponse().getStatus() != 200) {
            // If it's not 200, throw an exception to stop tests and highlight the issue
